...
status() {
       ...
       ps -ef | grep -v grep |  grep "$DIR/dist/starter.js" 2>/dev/null 1>/dev/null
       if [  $? -eq 0 ]
       then
          echo "Running"
       else
...
start() {
      ...
      then
        ...
        rm -f $DIR/nohup.out
        nohup $DIR/node_modules/.bin/supervisor -pid $DIR/tmp/PID.txt -w $DIR/dist,$DIR/conf -e js,yml -- $DIR/dist/starter.js $@ &
        sleep 3
